...
// Cache L1 global
Cache SimpleCache;
//uint8_t L2Cache[L2_SIZE]; // Cache L2 (simulação de memória)
uint8_t DRAM[DRAM_SIZE];  // DRAM (simulação de memória)
uint32_t time;
...
void accessL1(uint32_t address, uint8_t *data, uint32_t mode) {
    ...
    /** 
    index = (address >> 3);  // indice?
    Tag = address >> 3;               //  tag?
    */

    MemAddress = (address >> 3) << 3;       // Alinha o endereço ao início do bloco
    ...
    if (!Line->Valid || Line->Tag != Tag) { // Cache miss
        ...
        if (Line->Valid && Line->Dirty) {
            ...
            accessDRAM(oldMemAddress, Line->Data, MODE_WRITE);
        }
